...
const UploadArtPage = () => {
  ...
  const [newTitle, setNewTitle] = useState(art.title);
  const [newDescription, setNewDescription] = useState(art.description);
  const [newPainter, setNewPainter] = useState(art.painter);
  const [newTechnique, setNewTechnique] = useState(art.drawing_technique);
  const [newContent, setNewContent] = useState(art.content);
  const [newYear, setNewYear] = useState(art.work_year);
  const [newType, setNewType] = useState(art.type);
  ...
  return (
    <Wrapper>
      <TopBar />
      <UploadBtn onClick={editArt}>수정</UploadBtn>
      <ScrollArea>
        <Container>
          <ImgContainer>
            <UploadImg>
